...
export function openAICodexConflictMessage(): string {
  return 'dsh-imouto-codex cannot register provider "openai-codex" because another adapter already owns it. '
...
export async function diagnoseOpenAICodex(
  ...
  if (compatibility.status === 'incompatible') {
    hints.push('Compatibility mismatch: install the declared DSH plugin API versions and pin @earendil-works/pi-ai to 0.85.1, then run doctor again; no files are changed automatically.')
