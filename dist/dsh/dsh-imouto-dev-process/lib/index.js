import { createRequire as __imoutoCreateRequire } from 'node:module'; const require = __imoutoCreateRequire(import.meta.url);
...
function checkObjectSchemaTail(node, path3, properties, violations) {
  const hasRequired = Object.hasOwn(node, "required");
  const required2 = hasRequired ? node.required : void 0;
  if (hasRequired) {
    if (!isPlainJsonArray(required2) || required2.some((entry) => typeof entry !== "string")) {
      violations.push(`${path3}.required must be an array of strings`);
    } else {
      const declared = isJsonSchemaRecord(properties) ? properties : {};
      for (const key of required2) {
        if (!Object.hasOwn(declared, key)) violations.push(`${path3}.required names "${key}" which is not in properties`);
      }
    }
  }
  if (Object.hasOwn(node, "additionalProperties") && typeof node.additionalProperties !== "boolean") {
    violations.push(`${path3}.additionalProperties must be a boolean`);
  }
}
function checkSchemaNode(root, rootPath, violations, seen) {
  ...
  for (let task = tasks.pop(); task !== void 0; task = tasks.pop()) {
    ...
    const { node, path: path3 } = task;
    if (!isJsonSchemaRecord(node)) {
      violations.push(`${path3} must be a schema object`);
      continue;
    }
    if (seen.has(node)) {
      violations.push(`${path3} is circular`);
      continue;
    }
    seen.add(node);
    ...
    for (const key of Object.keys(node)) {
      ...
      if (ANNOTATION_KEYWORDS.has(key)) {
        try {
          if (!isJsonValue(node[key])) violations.push(`${path3}.${key} annotation must be lossless JSON data`);
        } catch {
          violations.push(`${path3}.${key} annotation must be lossless JSON data`);
        }
        continue;
      }
      violations.push(`${path3}.${key} is not a supported keyword (subset: type/oneOf/properties/required/additionalProperties/items/enum/const + annotations)`);
    }
    if (Object.hasOwn(node, "description") && typeof node.description !== "string") {
      violations.push(`${path3}.description must be a string`);
    }
    if (Object.hasOwn(node, "title") && typeof node.title !== "string") {
      violations.push(`${path3}.title must be a string`);
    }
    const hasType = Object.hasOwn(node, "type");
    const hasOneOf = Object.hasOwn(node, "oneOf");
    if (hasType && hasOneOf) {
      violations.push(`${path3} cannot declare both type and oneOf`);
      continue;
    }
    if (!hasType && !hasOneOf) {
      for (const key of ONE_OF_SIBLING_KEYWORDS) {
        if (Object.hasOwn(node, key)) violations.push(`${path3}.${key} requires type or oneOf`);
      }
      continue;
    }
    if (hasOneOf) {
      const oneOf = node.oneOf;
      tasks.push({ kind: "one-of-tail", node, path: path3 });
      if (!isPlainJsonArray(oneOf) || oneOf.length < 2) {
        violations.push(`${path3}.oneOf must be an array of at least two schemas`);
      } else {
        for (let index = oneOf.length - 1; index >= 0; index--) {
          tasks.push({ kind: "enter", node: oneOf[index], path: `${path3}.oneOf[${index}]` });
        }
      }
      continue;
    }
    ...
    if (typeof type !== "string" || !SCHEMA_TYPES.includes(type)) {
      violations.push(Array.isArray(type) ? `${path3}.type must be a single type string (type arrays are not supported)` : `${path3}.type must be one of ${SCHEMA_TYPES.join("/")}`);
      continue;
    }
    ...
    for (const [key, types] of Object.entries(allowedFor)) {
      if (Object.hasOwn(node, key) && !types.includes(schemaType)) {
        violations.push(`${path3}.${key} is not supported on type "${schemaType}"`);
      }
    }
    switch (schemaType) {
      case "object": {
        const properties = Object.hasOwn(node, "properties") ? node.properties : void 0;
        tasks.push({ kind: "object-tail", node, path: path3, properties });
        if (Object.hasOwn(node, "properties")) {
          if (!isJsonSchemaRecord(properties)) {
            violations.push(`${path3}.properties must be an object of schemas`);
          } else {
            const entries = Object.entries(properties);
            for (let index = entries.length - 1; index >= 0; index--) {
              const entry = entries[index];
              if (entry === void 0) continue;
              tasks.push({ kind: "enter", node: entry[1], path: `${path3}.properties.${entry[0]}` });
            }
          }
        }
        break;
      ...
      case "array": {
        if (Object.hasOwn(node, "items")) tasks.push({ kind: "enter", node: node.items, path: `${path3}.items` });
        break;
      }
      case "string":
      case "number":
      ...
      case "null": {
        ...
        if (hasEnum && !enumValid) {
          violations.push(`${path3}.enum must be a non-empty array of ${schemaType} values`);
        }
        ...
        if (hasConst) {
          if (!constValid) {
            violations.push(`${path3}.const must be a ${schemaType} value`);
          } else if (enumValid && !allowed.includes(declaredConst)) {
            violations.push(`${path3}.const must be one of ${path3}.enum when both are declared`);
          }
        }
        break;
      }
...
function diagnosticPath(path3) {
  return path3 === "" ? "arguments" : path3;
}
function propertyPath(path3, key) {
  return path3 === "" ? key : `${path3}.${key}`;
}
function losslessValueViolation(path3) {
  return [`"${diagnosticPath(path3)}" must be a lossless JSON value`];
}
function appendViolations(target, source) {
  for (const violation of source) target.push(violation);
}
function valueFrame(node, value, path3) {
  return {
    node,
    value,
    path: path3,
...
function checkScalarValue(node, value, path3) {
  const allowed = Object.hasOwn(node, "enum") ? node.enum : void 0;
  if (allowed !== void 0 && !allowed.includes(value)) {
    return [`"${diagnosticPath(path3)}" must be one of ${JSON.stringify(allowed)}`];
  }
  if (Object.hasOwn(node, "const") && value !== node.const) {
    return [`"${diagnosticPath(path3)}" must be ${JSON.stringify(node.const)}`];
  }
  return [];
}
function checkValue(schema, value, path3) {
  const frames = [valueFrame(schema, value, path3)];
  let rootResult;
  ...
  return rootResult ?? losslessValueViolation(path3);
}
function validateJsonSchemaValue(schema, value, path3 = "value") {
  return checkValue(schema, value, path3);
}
...
function assertAuthorKeys(source, path3, allowed) {
  for (const key of Object.keys(source)) {
    if (!allowed.includes(key)) authorError(`${path3}.${key} is not supported by the value schema DSL`);
  }
}
...
function runSchemaCompiler(initial) {
  ...
  for (let task = tasks.pop(); task !== void 0; task = tasks.pop()) {
    ...
    if (task.kind === "property-map") {
      ...
      tasks.push({ kind: "property-map-tail", compiled, required: required2, destination: task.destination });
    ...
    const { input, path: path3 } = task;
    if (!isJsonSchemaRecord(input)) authorError(`${path3} must be a value schema object`);
    if (seen.has(input)) authorError(`${path3} is circular`);
    seen.add(input);
    ...
    if (Object.hasOwn(input, "oneOf")) {
      assertAuthorKeys(input, path3, [...authorKeys, "oneOf", "type"]);
      if (Object.hasOwn(input, "type")) authorError(`${path3} cannot declare both type and oneOf`);
      if (!isPlainJsonArray(input.oneOf)) authorError(`${path3}.oneOf must be an array of at least two value schemas`);
      const branches = [];
      node.oneOf = branches;
    ...
    switch (inputType) {
      case "json":
        assertAuthorKeys(input, path3, [...authorKeys, "type"]);
        copyAnnotations(input, node);
        break;
      case "object":
        assertAuthorKeys(input, path3, [...authorKeys, "type", "properties", "additionalProperties"]);
        if (!Object.hasOwn(input, "additionalProperties") || typeof input.additionalProperties !== "boolean") {
          authorError(`${path3}.additionalProperties must be explicitly true or false`);
        }
        node.type = "object";
        copyAnnotations(input, node);
        ...
        if (Object.hasOwn(input, "properties")) {
          tasks.push({
            ...
            path: `${path3}.properties`,
            destination: { kind: "object", target: node }
          });
        }
        break;
      case "array":
        assertAuthorKeys(input, path3, [...authorKeys, "type", "items"]);
        node.type = "array";
        copyAnnotations(input, node);
      ...
      case "boolean":
      case "null":
        assertAuthorKeys(input, path3, [...authorKeys, "type", "enum", "const"]);
        node.type = inputType;
        copyAnnotations(input, node);
        if (Object.hasOwn(input, "enum")) {
          if (!isPlainJsonArray(input.enum)) authorError(`${path3}.enum must be a non-empty array of scalar values`);
          node.enum = Array.from(input.enum, (entry) => entry);
        }
        if (Object.hasOwn(input, "const")) node.const = input.const;
        break;
      default:
        authorError(`${path3}.type must be string/number/integer/boolean/null/array/object/json, or use oneOf`);
    }
  }
}
function compilePropertyMap(input, path3) {
  const holder = {};
  runSchemaCompiler({ kind: "property-map", input, path: path3, destination: { kind: "root", holder } });
  return holder.value ?? authorError(`${path3} did not compile`);
}
function compileValueSchema(input, path3) {
  const holder = {};
  runSchemaCompiler({ kind: "value", input, path: path3, allowRequired: false, destination: { kind: "root", holder } });
  return holder.value ?? authorError(`${path3} did not compile`);
}
...
function defineTool(options) {
  ...
  if (userPresentCall) {
    tool.presentCall = (args) => {
      if (validate2(args).length > 0) return void 0;
  ...
  if (userIsConcurrencySafe) {
    tool.isConcurrencySafe = (args) => {
      if (validate2(args).length > 0) return false;
...
// tools/blackboard/rules.ts
import path from "node:path";

// tools/blackboard/parse.ts
var import_yaml = __toESM(require_dist(), 1);
function parse(text) {
  const source = text.replace(/^\uFEFF/, "");
  const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n/.exec(source);
  if (!match) throw new Error("missing frontmatter");
  return { data: import_yaml.default.parse(match[1] ?? ""), body: source.slice(match[0].length), frontmatter: match[1] ?? "", offset: 2 };
}
...
// tools/blackboard/rules.ts
var required = ["protocol", "task", "initiator", "execution_mode", "status", "owner", "next_action", "created", "updated", "project", "base_revision"];
var enums = { initiator: ["Chloe", "Sherry", "Yuu", "oniichan"], execution_mode: ["solo", "delegated", "auto"], coordination_transport: ["native", "discord", "driver"], status: ["planning", "ready", "implementing", "review", "verifying", "done", "blocked"], owner: ["Chloe", "Sherry", "Yuu", "oniichan", "none"] };
function section(body, name2) {
  return new RegExp(`^## ${name2}\\r?\\n([\\s\\S]*?)(?=^## |(?![\\s\\S]))`, "m").exec(body)?.[1];
}
function finding(rule, severity, line, message) {
  return { rule, severity, line, message };
}
function validate(file, text) {
  ...
  try {
    ...
    const source = text.replace(/^\uFEFF/, "");
    return source.startsWith("---\n") || source.startsWith("---\r\n") ? [finding("fm-parse", "error", 1, String(error))] : "legacy";
  }
  ...
  for (const name2 of ["Objective", "Acceptance Test", "Plan", "Subimoutos", "Thread"]) if (section(parsed.body, name2) === void 0) out.push(finding("section-required", "error", 1, `missing ${name2}`));
  for (const name2 of ["Work Items", "Implementation Notes", "Review"]) if (section(parsed.body, name2) === void 0) out.push(finding("section-expected", "warn", 1, `missing ${name2}`));
  ...
  if (subs !== void 0 && subs.trim() !== "None.") {
    ...
    for (const block of blocks) {
      const body = block[1] ?? "";
      const values = Object.fromEntries([...body.matchAll(/^- ([^:]+): (.*)$/gm)].map((m) => [m[1], m[2]]));
...
// tools/blackboard/transition.ts
var edges = { planning: ["ready", "implementing", "blocked"], ready: ["implementing", "blocked"], implementing: ["review", "blocked"], review: ["implementing", "verifying", "done", "blocked"], verifying: ["implementing", "done", "blocked"], blocked: ["planning", "ready", "implementing", "review"], done: [] };
function transition(text, input) {
  const prior = validate("task.md", text);
  if (prior === "legacy") throw new Error("legacy file");
  const current = /^status: (.+)$/m.exec(text)?.[1];
  ...
  const owner = input.to === "done" ? "none" : input.owner;
  const next = input.to === "done" ? "none" : input.nextAction;
...
verifier: ${input.verifier}`);
  ...
  const after = validate("task.md", result);
  if (after === "legacy") throw new Error("invalid result");
  const oldErrors = new Set(prior.filter((f) => f.severity === "error").map((f) => `${f.rule}:${f.message}`));
  const added = after.filter((f) => f.severity === "error" && !oldErrors.has(`${f.rule}:${f.message}`));
  if (added.length) throw new Error(added[0]?.message);
  return result;
}

// tools/dsh-plugin.ts
var name = "imouto-blackboard";
var inject = ["tools"];
function markdownPath(value) {
  if (!path2.isAbsolute(value) || path2.extname(value).toLowerCase() !== ".md") throw new Error("path must be an absolute .md path");
  return value;
}
function validationLines(file) {
...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args) => {
      ...
      const result = transition(fs.readFileSync(file, "utf8"), { to: args.to, owner: args.owner, nextAction: args.next_action, verifier: args.verifier, today: (/* @__PURE__ */ new Date()).toISOString().slice(0, 10) });
